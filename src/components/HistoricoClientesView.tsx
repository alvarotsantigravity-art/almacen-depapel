    ...
    <div className="space-y-6">
                ...
                albaranes.map((alb) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* N° Albarán y Badge de Personalizado */}
                      <td className="p-3.5">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-mono font-bold text-[#1e1e1e] whitespace-nowrap">
                            {alb.numero_albaran}
                          </span>
                      ...
                      <td className="p-3.5 text-right pr-6">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            onClick={() => exportToPDF([alb], `Albaran_${alb.numero_albaran}.pdf`)}
                            title="Generar PDF"
      ...
      {selectedAlbaranDetail && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-[#ccd1da] max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-xl space-y-6 animate-fade-in-up">
            <div className="flex items-start justify-between gap-4 border-b border-[#ccd1da]/60 pb-4">
              <div>
                <div className="text-xs font-semibold text-[#0098f2] uppercase tracking-wider mb-1">
                  Detalle de Albarán
                </div>
                <h3 className="text-2xl font-extrabold text-[#1e1e1e] tracking-tight">
                  {selectedAlbaranDetail.cliente.nombre_empresa}
                </h3>
                <p className="text-xs text-[#666666]">
                  N° Albarán: <span className="font-mono font-bold text-[#1e1e1e]">{selectedAlbaranDetail.numero_albaran}</span> • Fecha: {formatDisplayDate(selectedAlbaranDetail.fecha)}
                </p>
              </div>

              <button
                onClick={() => setSelectedAlbaranDetail(null)}
                className="p-2 text-[#666666] hover:text-[#1e1e1e] hover:bg-[#f7fafc] rounded-full transition cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2 flex-wrap">
              ...
              <button
                onClick={() => exportToPDF([selectedAlbaranDetail], `Albaran_${selectedAlbaranDetail.numero_albaran}.pdf`)}
